...
class CorpseHttp
  ...
  def view
    ...
    if term.name.like("horaire")
      ...
      <p>The average daily {_Fh_}, or the {*Hour Day Focus*} {_Hdf_}, is an index of average focus in a specific project. In the above graph, the average {_Fh_}, of #{home_term.logs.hours}{_Fh_} over #{home_term.logs.length} days, is equal to #{home_term.logs.hour_day_focus}{_Hdf_}. Its maximal value is 9, so the Hdf ratio is currently of #{home_term.logs.hour_day_focus_percentage}%.</p>
      <h2>HTo/HTa</h2>
      #{Graph_Yearly.new(home_term,0,365)}
      <mini>Sectors for the previous 13 months.</mini>
      <p>The {*Hour Topic*} {_HTo_} & the {*Hour Task*} {_HTa_} - where {_HTo_} is the sum of {_Fh_} over the number of different topics, or #{home_term.logs.hours}{_Fh_} over #{home_term.logs.topics.length} topics, and {_HTa_} the sum of logged {_Fh_} over the number of different tasks, or #{home_term.logs.hours}{_Fh_} over #{home_term.logs.tasks.length} tasks.</p>".markup
      return html
