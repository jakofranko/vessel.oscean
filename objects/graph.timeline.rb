...
class Graph_Timeline
  ...
  def to_s
    ...
    markers += "<span style='position:absolute; bottom:15px;left:130px'><tt style='color:red; padding-right:5px'>— </tt> Visual <span style='color:#999'>#{@sums[:visual] ? ((@sums[:visual].to_i/@sumHours.to_f)*100).to_i : 0}%</span></span>"
    markers += "<span style='position:absolute; bottom:15px;left:230px'><tt style='color:white; padding-right:5px'>— </tt> Research <span style='color:#999'>#{@sums[:research] ? ((@sums[:research].to_i/@sumHours.to_f)*100).to_i : 0}%</span></span>"
