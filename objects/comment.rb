...
class Comment
  ...
  def to_s
    ...
    return "<comment "+(nest ? "class='nested'" : "")+">"+(message ? message : "This comment has been deleted.")+" <span class='timestamp reply' data='#{nest ? nest : id}'>#{timestamp.ago}</span>"+(author ? " by ~#{author}" : "")+"</comment>\n"
